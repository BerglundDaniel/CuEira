...
\begin{titlepage}
    \begin{center}
        ...
        \Huge
        \textbf{Gene-Environment Interaction Analysis Using Graphic Cards}
        
        \vspace{0.5cm}
        \LARGE
        Gen-miljö interaktion analys med grafikkort
        
        \vspace{1.5cm}
        
        \textbf{Daniel Berglund}\\
        \textbf{8 March 2015}\\
        dabergl@kth.se
        ...
        Project Provider: Henrik Källberg
...
In the case when the outcome is a disease variables with odds ratio below one are called \emph{protective} and when it is above one it is a \emph{risk factor}~\cite{recoding_2011}.
...
\begin{itemize}
 \item \emph{SISD}(Single-Instruction,Single-Data) is the sequential and is as the name suggests single instructions on single data~\cite{introduction_hpc_hager}.
...
Some programming languages, e.g. Fortran, disallow some types of aliasing while others, e.g. C/C++, allows aliasing~\cite{introduction_hpc_hager}. The compilers for the languages in the second case commonly have an option to disable aliasing. However for languages that do not allow aliasing, or if it is disabled, it is then the programmers responsibility to make sure that aliasing does not occur or the results can be wrong~\cite{introduction_hpc_hager}.
...
\\
GPUs are made to process images in a fast manner so that games and similar tasks runs smoothly~\cite{cuda}. Because GPUs are made for image processing they are GPUs have a large number of cores(~1000-2000)~\cite{cuda, nvtesla}. Each core is slower than a core in a CPU, however the number of cores makes up for it. The downside is that it is slower than a normal CPU for sequential tasks so the program has to take advantage of the extreme parallelism to get good speed~\cite{cuda_best_practice}. The memory of the GPU is separate from the computers main memory which means that the data needs to be transferred between the memories which increases the complexity of the program~\cite{cuda}.\\
\\
...
\begin{description}
  ...
  \item[Use fast math functions if precision is not needed] \hfill \\
  There are three versions of the math functions. The double precision version is func() while the single precision function is funcf(). There is a third, faster, however less accurate version \_funcf(). The option -use\_fast\_math makes the compiler change all the funcf() to \_funcf().~\cite{cuda_best_practice}
...
\begin{figure}[h]
    ...
    \caption[Repeatability in computer science]{Repeatability in computer science,from~\cite{comp_repro_2013}}
...
\\
Speed up is a measure of how much faster then program is with a certain number of threads compared to the serial version. It is defined as~\cite{introduction_hpc_hager}
...
\\
Efficiency reflects how efficient the program is using p threads. Linear speed has efficiency 1. Efficiency is defined as~\cite{introduction_hpc_hager}
...
Amdahl's Law is used to find the expected speedup of a system when parts of it are made concurrent~\cite{2010_reevaluating_amdahl}. Simply it states that as the number of processors increases the parts that are not parallel will start taking up more and more of the wall clock time and that the speedup for adding more processors will decrease as more and more processors are added and more time is spent relatively on the non parallel parts~\cite{introduction_hpc_hager, 2010_reevaluating_amdahl}. It is closely related to strong scaling~\cite{cuda_best_practice,2010_reevaluating_amdahl}.\\
\\
Amdahl's law states the expected speedup with F fraction of the code parallel and p threads is~\cite{introduction_hpc_hager} 
...
\begin{itemize}
  ...
  \item The serial portion can not be overlapped by the parallel parts. For instance with producer consumer pattern the consumer could be strictly serial but the time consumer takes could be overlapped by the parallel producers.
\end{itemize}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=10cm]{AmdahlsLaw.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=12cm]{GustafsonsLaw.png}
...
\\
\emph{Exhaustive search} is the most direct approach, it compares all combinations of the SNPs in the dataset. Exhaustive search methods will not miss a significant combination because it did not consider that specific combination. However it also means that they can be slow since they will spend time on combinations other methods would skip completely. Multifactor-Dimensionality Reduction(MDR)~\cite{mdr_2001} and BOOST~\cite{boost_gene_gene} are two examples of this type of algorithm.\\
\\
...
Finding the $\beta$ coefficients are done in a similar way as with other linear regression models since they all are generalized linear models~\cite{agresti_categorical}. It is usually done using  maximum likelihood(ML) via Newtons method~\cite{agresti_categorical, uvehag_master_thesis}. It is an iterative method which means it can be relatively slow compared to non iterative methods. The pseudo code for the algorithm can be found in algorithm~\ref{alg:lr}~\cite{uvehag_master_thesis}. Line 13 is sometimes inside the loop, however since it is not needed for the actual iteration calculating it for every loop wastes time.
...
\label{matrix_inverse} %TODO U och V unitära matriser?
...
The descriptions and structures in this section applies to both JEIRA and GEISA since their underlying structure is the same. Both programs are written in Java and uses Javas features for concurrency. GEISA has three dependencies, two are provided as JAR files in the distribution so the user does not need to get install them separately. The dependency Apache Maven is only needed if the user wants to compile the program from source.
...
\section{CuEira}
\label{cueira}
This section will explain the structure of CuEira and how it works starting with a general overview. Later the parts are explained and finally how the parts are combined to form the program. C++ was chosen because it is generally fast and has good control of the memory. The downside is that it is more platform dependent than Java. Java can also be easier to work with in several ways.\\
\\
...
CuEira has some dependencies. The dependency on MKL and Intels compiler is something that can be changed. BLAS shares the interface so another BLAS library could work with updates to the CMake file.
...
\begin{description}
  ...
  \item[CUDA] \hfill \\
  NVIDIAs library for GPU programming, explained in section~\ref{cuda_programming_model}
  \item[CUBLAS] \hfill \\
  BLAS for CUDA
  \item[MKL] \hfill \\
  Intels BLAS library
...
\subsection{Storage}
A set of container classes hold the data. There are two types of them, the basic matrix and vector classes and data containers. The data containers are responsible for handling the data for the model and performing the recoding. They use the vector classes for the actual storage.\\
\\
...
\\
The \textbf{EnvironmentFactorHandler} is responsible for keeping track of the \textbf{EnvironmentFactors} and its corresponding data. It also provides functions to access the \textbf{EnvironmentFactors} and the corresponding vectors.\\
\\
There are three data container classes, \textbf{SNPVector}, \textbf{EnvironmentVector} and \textbf{InteractionVector}. They all have the responsibility to keep track of the recoded data and to perform the recoding when their \textbf{recode} function is used. The \textbf{recode} function takes a \textbf{Recode} enum which tells it which type of recoding it should perform. The \textbf{SNPVector} also holds the original SNP data. \textbf{EnvironmentVector} does not need to since \textbf{EnvironmentFactorHandler} has that responsibility and \textbf{InteractionVector} does not have any kind of original data.\\
\\
...
\\
There is only one writer, the \textbf{ResultWriter}, and as its name suggests its responsibility is to write the results. The results are written immediately and then discarded to save memory. There is a lock on the writing so several threads can share it. The results for each combination will therefore be written in the order they are completed. The format of the output file is described in appendix~\ref{file_formats}.
...
\\
As is sometimes common BLAS function names are heavily abbreviated. For instance a function for matrix vector multiplication is \textbf{sgemv}, the $s$ stands for single precision, $ge$ for general, $m$ for matrix and $v$ for vector. General means that it is standard matrices and vectors, e.g. not triangular or sparse. The equivalent function in \textbf{MKLWrapper} can be found in example~\ref{ex:mkl_wrap_mv}. The name matrixVectorMultiply instantly tells the user what it does. As mentioned earlier in section~\ref{cueira} the CuEira is either single or double precision depending on an option when compiling so \textbf{MKLWrapper} does not mention the precision. All matrices and vectors are also general. However even if the function name would contain that information too it would still not be so long that it was in the way. A possible name by using namespaces for the precision and type could be General::SinglePrecision::matrixVectorMultiply.\\
...
\\
The transfers to and from the device are done by two classes, \textbf{DeviceToHost} and \textbf{HostToDevice}. They perform asynchronous transfers on the corresponding matrix and vector class. It can either create and transfer to a new container or transfer to a given place in the memory. The former is useful for instance when transferring a group of vectors into a combined matrix or an already existing container.
...
The variables used on the device and their number of elements are shown in table~\ref{table:gpu_variable_sizes}. The memory need with $T$ number of streams is shown in equation~\ref{eq:device_memory}.The number of individuals than can fit inside 6GB(i.e. a typical GPU) with three streams for different number of covariates and precision is shown in table~\ref{table:gpu_mem}. The number of individuals possible is much higher than in a typical study.
...
The number of streams affects the performance significantly. As figure~\ref{fig:gpu_10streams_10ks_10ki} and figure~\ref{fig:gpu_10streams_10ks_100ki} show using too few streams increases the time the program takes. This is because to few streams does not provide enough work for the GPU. The best performance is when the total number of streams is slightly above the number of cores. It likely means that the CPU is the bottleneck. However for some data sizes with one GPU it is possible that the GPU is the bottleneck as shown in figure~\ref{fig:gpu_10streams_10ks_10ki} where the increase in speed stop as six streams.\\
\\
...
\begin{figure}[h]
    ...
    \caption{Varying the number of GPUs and individuals. Number of streams are 9,4,3,2 for 1 to 4 GPUs, 10 000 SNPs, 0 covariates}
...
%\begin{figure}[h]
    ...
    \caption{Varying the number of GPUs and covariates. Number of streams are 9,4,3,2 for 1 to 4 GPUs, 10 000 SNPs, 2 000 individuals}
...
Table~\ref{table:gpu_profile_calc_dist} shows how the time for doing calculations on the GPU is used based on the output from the profiler Nsight. Most of the time is spent on gemv2T which is matrix multiplication of a transposed matrix and a vector. It is used in the calculation of the scores vector, line 8 in algorithm~\ref{alg:lr}. The custom kernels takes up a small part of the calculations unless the number of covariates is high, see table~\ref{table:gpu_profile_calc_dist_cov}. About 10\% of the time is spent on the element wise multiplication. For higher number of GPUs Nsight showed warning messages about low overlap between transfers and kernels and between kernels and kernels.
...
\begin{figure}[h]
    ...
    \caption{Distribution of time spent on the LR algorithm. 1 GPU, 9 streams, 10 000 SNPs, 0 covariates}
...
\begin{figure}[h]
    ...
    \caption{Distribution of time spent on the LR algorithm with increased synchronisations. 1 GPU, 9 streams, 10 000 SNPs, 0 covariates}
...
\\
As previously mentioned in earlier chapters, CuEira has several compiling options that change some of its properties. The results show that single precision has better performance than double precision in almost all cases for the program. Increased synchronisation can give better performance when the number of individuals is 100 000 and more. Increased synchronisation does decrease the transfer times but it does not always result in better performance.\\
\\
...
\begin{tabular}{| l | c c c | c | c c c |}
  ...
  \hline  
\end{tabular}
\caption[The four cases of allele frequencies]{The four cases of allele frequencies. Case one upper left, case two upper right, case three lower left, case four lower right}
\label{table:risk_allele_cases}
\end{table}

By using the previously mentioned definitions in section~\ref{risk_allele} and the JEIRAs and GEISAs algorithm~\ref{alg:jeira_geisa_risk_allele} what the risk allele is according to them for each case is shown in table~\ref{table:risk_allele_comp}. Each definition correspond to examining the inequalities in the cases. For the definition using MAF the direction of the inequality from case minimum to control minimum determines it. If it points towards control the the case minor allele is the risk allele. For JEIRA and GEISA if the maximum for case points straight down to the maximum for control then the major case allele is the risk allele. This only happens in case one. For increased frequency in case it is the allele which has the inequality pointing straight down.\\

\begin{table}[h]
\centering
\begin{tabular}{| l | c c c c |}
  \hline
  Definition & Case one & Case two & Case three & Case four  \\
  \hline
...
The risk allele is different for case three and case four. For case three it is clear that Allele1 likely carries the risk, because it has the highest frequency among cases and has higher frequency in case than control An extreme case of case three is shown in table~\ref{table:risk_allele_extreme_case_3}.\\
\\
By examining a similar extreme case of case four it is clear that Allele1 likely carries the risk in that case too. An corresponding extreme case is shown in table~\ref{table:risk_allele_extreme_case_4}. Almost everyone in the case group has Allele1 while no one in the control group does, it is then unlikely that Allele2 would increase the risk.
...
\caption{Two extreme cases of allele frequencies based the cases in on table~\ref{table:risk_allele_cases}}
\end{table}
...
\includepdf[pages={2}]{kth_cover.pdf}
\end{document}