...
%TODO cite mattrix inverse stuff

The inverse of the \emph{information matrix}, \textbf{J}, on line 10 in algorithm \ref{alg:lr} is generally not possible to do with normal matrix inversion because it is not defined for a general matrix. However the \emph{pseudoinverse} is defined for a general matrix, it is denoted as $A^+$ for the matrix $A$. There are several types of pseudoinverse, one of the more common is the \emph{Moore-Penrose pseudoinverse} where $A^+$ is the matrix that satisfies equations \ref{eq:pseudo_1} to \ref{eq:pseudo_4}.
...
\emph{Matrix decomposition}, also called \emph{matrix factorisation}, are methods to factorize a matrix into products of matrices. Some of the decomposition methods can be used to find the pseudoinverse. \emph{Singular value decomposition}(SVD) is one of them and works for general matrices. It's factorization of a  $n \times m$ matrix $A$ is shown in equation \ref{eq:svd}.

\begin{equation}\label{eq:svd}
A=U \Sigma V^T
\end{equation}

Where $\Sigma$ is a diagonal matrix with non negative numbers, its diagonal values are the \emph{singular values} of $A$. Using SVD the pseudoinverse is shown in equation \ref{eq:pseudo_svd}. $\Sigma^+$ is the pseudoinverse of $\Sigma$. This is done by inverting each non zero element of the diagonal and transposing the matrix.
...
\section{CuEira}
\label{cueira}
This section will explain the structure of CuEira and how it works starting with a general overview. Later the parts are explained and finally how the parts are combined to form the program. C++ was chosen because it is generally fast and have good memory handling.\\ %TODO more about C++ reasons?
\\
...
\\
The CMake file contains several options for how CuEira can be compiled. For instance one of them sets if it should be single or double precision. Others tell the program information about the target GPU architecture, debug mode and so on. More details on how to compile and use CuEira can be found in appendix \ref{compile_cueira}.
...
CuEira reads five files that are in four different formats. There is one class for each file that takes care of reading the files contents.\\
\\
Three of the files are PLINK files in binary format. The fam file has the information about the individuals, the bim file has the names of the SNPs and their alleles. The genetic information for each individual is in the binary bed file. More details on PLINK files can be found in appendix \ref{plink_file}. The classes used to read them are BimReader, BedReader and FamReader.\\
\\
...
LR algorithm is split into several protected functions so that each part can be tested individually. The parts are then used in the \textbf{calculate} function. For CudaLogisticRegression the parts corresponding to line 10 and 11 in algorithm \ref{alg:lr} are done on the host. All the other parts of the LR algorithm are done by using CUBLAS or the kernels explained in section \ref{kernelwrapper}. The calculations for line 10 and 11 are in the LogisticRegression class so the code is shared between CpuLogisticRegression and CudaLogisticRegression. Line 10 and 11 are done on the host instead of the device because two reasons.\\
\\
The size of the vectors and matrices used there is small, their size is 4 + the number of covariates. The second reasons is that the inverse of the information matrix(variable \textbf{J} in the algorithm) is done by SVD. The reason for using SVD instead of normal matrix inverse was previously explained in section \ref{logistic_regression}. However CUBLAS does not provide an SVD function. There are other libraries that do, CULA Tools\cite{culatools} and MAGMA\cite{magma_2010}, however neither could be used.\\
\\
...
Main described in figure \ref{fig:main_layout}

Thread described in figure \ref{fig:thread_layout}

\begin{figure}[h]
    \centering
    \includegraphics[width=3cm]{main_layout.png}
    \caption{Structure of main thread}
...
\subsection{Timers}
%TODO write about timers stuff
There is an option in the CMake file to add clocks that measures how long time different parts of the code takes. 
...
\section{CuEira vs GEISA}
Bla bla bla

\begin{figure}[h]
    \centering
    \includegraphics[width=12cm]{geisa_vs_cueira_nocluster_10ksnps.png}
    \caption{Speedup GEISA vs CuEira. 10 000 SNPs.}
    \label{fig:geisa_vs_cueira_nocluster_10ksnps}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{Speedup vs 1 GPU. 10 000 SNPs.}
...
\begin{figure}[h]
    ...
    \caption{Efficiency vs 1 GPU. 10 000 SNPs.}
...
\begin{figure}[h]
    ...
    \caption{Execution time with 4 GPUs with either synchronisation or not. 10 00 SNPs.}
    \label{fig:sync_vs_no_sync}
\end{figure}
...
The allele frequencies can be shown in an 2x2 table. The row sums always sums to 1 because it has to be 100\%.
It can be split in 4 cases depending on where the max and min is. Due to rotations(it's the same if the alleles are switched).
Red arrow is the interesting one, the rest are based on it
...
For the definition in section \ref{risk_allele} using MAF. The arrow from min to min sets the risk. If the case points to control then case minor is risk.

For JEIRA and GEISA if the M for case points down to M for control then it is risk. This only happens in case 1. All the others either fail at max case < max control or not same allele

For increase frequency in case vs control it is the vertical arrow pointing towards control, i.e. case is larger

\begin{table}[h]
\centering
\begin{tabular}{| l | c c c c |}
  \hline
  Definition & Case 1 & Case 3 & Case 2 & Case 4  \\
  \hline
  MAF & Allele1 & Allele2 & Allele1 & Allele2 \\
  JEIRA GEISA code & Allele1 & Allele2 & Allele2 & Allele2 \\
  Increased in case & Allele1 & Allele2 & Allele1 & Allele1 \\
  \hline  
\end{tabular}
...
Looking at case 4 because different there. Extreme case: 0.9 1 0 1
Clearly allele 1 is risk, but maf sets allele 2
