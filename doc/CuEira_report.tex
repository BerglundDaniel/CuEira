...
%TODO write about inheritance, namespaces, pointers, memory, enums
...
All the programs read Plink data files except CuEira which can only read the binary Plink files. They also read a separate file that contains the environmental data and covariates if any. For CuEira the covariates and environmental data is split in two different files with the same structure. %TODO cite plink file stuff
...
\\
By changing an option in the main CMake file the program can be compiled with either single and double precision. More specific details on how to compile CuEira can be found in appendix \ref{compile_cueira}.\\ %TODO more preproccessor variable stuff, PRECISION, DEBUG, CPU
\\
...
DataHandler %TODO probably not here
...
\\
MKL, e.g. Intels BLAS is wrapped in MKLWrapper. The BLAS interface standard is not object oriented which makes %TODO
...
One time during the thesis it took several days to find an error that was in one of the CUBLAS calls. Due to a 1 instead of 0 it performed the operation $y=z*x+y$ instead of $y=z*x$. These types of errors are much easier to make when a function has a larger number of similar parameters.\\
...
CUBLAS is wrapped together with the created kernels in KernelWrapper in a similar way to how MKL is wrapped in MKLWrapper. KernelWrapper is explained in \ref{kernelwrapper} Other CUBLAS utility and CUDA functions are wrapped in CudaAdapter.

Stream class

Device class

The transfers to and from the device is handle by two classes, DeviceToHost and HostToDevice. They perform asynchronous transfers on the corresponding matrix and vector class. It can either create a new container to transfer to or transfer to an given place in the memory. The former is useful for instance when transferring different vectors to into a combined matrix.
