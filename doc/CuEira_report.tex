...
\\
A position of the DNA or a gene is a \emph{locus}\cite{sadava_life}. A variation of the same gene or locus is an \emph{allele}\cite{sadava_life}. The effect of an allele on a phenotype can be either \emph{dominant}, \emph{recessive} or \emph{co-dominant}. Dominant means that the effect occurs if the allele only needs to be present in one of the chromosomes in the pair, recessive is when the allele needs to be present in both chromosomes \cite{sadava_life}. Co-dominant is when both alleles effect the phenotype, when different alleles are present this usually produce some kind of intermediate state\cite{sadava_life}.
%TODO fix it, dominat, reccessive etc
...
%TODO inte helt korrekt? eftersom baseras på additive interaction så kan inte placeras här? interaction ej definerad än
Based on the statistcal model and its corresponding ORs there are some measures that can be calculated. These measures show various properties of the additive interaction. They are defined using $RR$ however as mentioned before in section \ref{rr_or} $OR$ can be used to approximate $RR$.\\
\\
...
Central processing unit(CPU) is the part of the computer that executes instructions one by one in its core\cite{introduction_hpc_hager}.%TODO what is core
Most modern CPUs have multi-core architecture\cite{introduction_hpc_hager}. Each core can perform tasks independent of each other.
\\
...
In section 6.2.1 in What Every Programmer Should Know About Memory\cite{drepper2007cpumemory} an example of how much speed that can be gained by optimizing matrix matrix multiplication. The simple version of it is shown in algorithm \ref{alg:matmatmult} and the version with prefectching is shown in \ref{alg:matmatmultopt}
...
\end{algorithmCode}
%TODO describe variables and things here under algorithm
...
//All variables are pointers %TODO move to below as comment
*a = *b + *c;
*d = *e + *f;
\end{example}
...
This section starts with the algorithms and then the implemenations.
...
Example of recode diference on data sample
