...
%TODO fix device mem intro
The variables used on the device and their number of elements are shown in table \ref{table:gpu_variable_sizes}. The memory need with $T$ number of streams is then

\begin{equation}\label{eq:device_memory}
(N+T(2NM+2N+2M+MM))*Element Size
\end{equation}

The amount of individuals than can fit inside 6GB(i.e. a typical GPU) with three streams for different number of covariates and precision is shown in table \ref{table:gpu_mem}.\\
\\
...
The simulated data consists of randomized data with equal number of cases and controls.
...
\begin{figure}[h]
    ...
    \caption{Same as figure \ref{fig:10stream_init} but calculating in opposite order.}
...
SNPs
Therefore only 10k snps

seconds plot

reltive thing for it

Individuals increase work but it does not increase so that double means double the work, shown in figure \ref{fig:individuals_cov_0_stream_4}. There are no major changes with the addition of covariates. With 20 covariates is shown in figure \ref{fig:individuals_cov_20_stream_4}.

\begin{figure}[h]
    \centering
    \includegraphics[width=9cm]{individuals_cov_0_stream_4.png}
    \caption{10ks 0 cov 4 stream}
...
Covariates
large effect for small number of individuals, little to no effect on 200k i, see figure \ref{fig:cov_individuals2000_stream_4} and figure \ref{fig:cov_individuals2e+05_stream_4}
...
relative thing for cov

\section{Streams and Multi-GPU}
as streams increase GPU gets full for 1 GPU
however cpu full when streams*core>8
shows cpu side is problem when >2 gpu, possibly for 2 gpu too

number of streams with best speed is so that the number of streams * number of GPUs is slightly above the number of cores. However to many streams not a problem as long as memory is enough, slightly speed loss. It also depends on the data sizes. At some point the GPU will be full as figure \ref{fig:gpu_10streams_10ks_10ki} shows.

A more zoomed in to see what happens with 5+ streams and 100k individuals is shown in figure \ref{fig:gpu_5to10streams_10ks_100ki}. The speed flattens out with nine streams which is one more than the number of cores.

\begin{figure}[h]
    \centering
    \includegraphics[width=9cm]{gpu_10streams_10ks_10ki.png}
    \caption{1 to 10 streams with 10 000 SNPs and 10 000 individuals}
...
\begin{figure}[h]
    ...
    \caption{1 to 10 streams with 10 000 SNPs and 100 000 individuals}
...
\begin{figure}[h]
    ...
    \caption{5 to 10 streams with 10 000 SNPs and 100 000 individuals}
    \label{fig:gpu_5to10streams_10ks_100ki}
\end{figure}

for satured streams:
for both cov and ind as x axis
...
Using double precision increases the time in almost all cases. Multiple runs with 10 000 SNPs and varying number of individuals, covariates, GPUs and streams are shown in figure \ref{fig:single_vs_double}. Three points are significantly faster with double precision than single. However almost all the other points have reduced speed so using double precision is an bad idea. It is also possible that three points are outliers. %TODO something about the increase in snp reading time, rerun the three points

\begin{figure}[h]
    \centering
    \includegraphics[width=10cm]{double_calc.png}
    \caption{16 points inside each group}
    \label{fig:single_vs_double}
\end{figure}

\section{Syncing}
This relates to the queue problem described in section \ref{streams}

increased with more syncing except for large number of individuals

interesting lines, higher number of gpu and streams gives better gain
it is the calculation time so no due to initialisation thing

\begin{figure}[h]
    \centering
    \includegraphics[width=10cm]{ferm_calc.png}
    \caption{16 points inside each group}
    \label{fig:less_sync_vs_always_sync}
\end{figure}

Plot of the LR time dist with and without sync, 200ki 10ks, also 10ki, show that transfer times disappear for all num of individuals

The fraction of the LR time spent on transfers are much lower with syncing as shown in figure ???
But why is it only faster with 100k+ individuals then?

possible explanation, gpu is full in both cases so same overall time, but where the wait time ends up differ
without sync the transfer takes more time because behind kernel blocking
with sync it doesn't get stuck instead kernel needs to wait for gpu to complete other kernel from other stream

\section{Profiling}
It is not 
Because of asynchronous and threads can be hard to draw conclusions even if one part takes time. Because can be hidden by GPU
good example with the syncing, transfer times decrease but overall remains the same

with 4 gpu 3 stream nsight complains that overlap of kernel with kernel and mem transfer with transfer is bad, only a couple of \%
does not happen with 2 gpu 4 stream

Table \ref{table:gpu_profile_calc_dist} shows how the time for doing calculations on the GPU is used. Most of the time is spent on gemv2T which is matrix multiplication of a transposed and a normal matrix. It is used in the calculation of the information matrix.\\
\\
Since the custom kernel takes so little time there is small gains in optimizing them with either unrolling or varying block size. That time is likely better spent on optimizing a different part.

%TODO add 5 10 20 cov
% test diferent num of individuals too, 100k??, 10k

\begin{table}[h]
\centering
\begin{tabular}{| l | c c |}
  \hline
  Percentage time spent & Kernel & Kernel type  \\
  \hline
  72.1 & gemv2T & CUBLAS\\
  14.4 & block dot & CUBLAS\\
  8.1 & gemv2N & CUBLAS\\
  2.3 & ElementWiseMultiplication & Custom\\
  0.9 & LogisticTransform & Custom\\
  0.7 & LogLikelihoodParts & Custom\\
  0.6 & ElementWiseDifference & Custom\\
  0.5 & VectorMultiply1MinusVector & Custom\\
  0.3 & dot kernel & CUBLAS\\
  0.1 & reduce & CUBLAS\\
  \hline  
\end{tabular}
\caption{Distribution of the time spent on GPU calculations}
\label{table:gpu_profile_calc_dist}
\end{table}

%TODO run with sync check lines stuff

The \textbf{ResultFileWriter} and \textbf{DataQueue} each has a lock to prevent race conditions. They also have a timer for how much time that has been spent waiting at the locks. There was very little time spent at them, not more than a couple of seconds even for the larger files which takes 10-30 mins to perform. Overall very insignificant time
The next sections will focus on some of the other parts.

\subsection{ModelHandler and DataHandler}
The time spent 
some time on recode and app model, most of there time was spent reading the SNP. With more GPUs the read time is increased

hard to say if problem because threads so possibly hidden

plot relative time \% spent vs gpu
seconds plot vs gpu
10k and 100k individuals

possibly plot read vs LR tot
is the time to read a lot or little compared to LR GPU?

plot next+trans+cpu vs LR gpu

\subsection{LR}
As figure \ref{fig:timedist_LR_cov_10ks_2000i_gpu4} shows with increased number of covariates the CPU time of the LR algorithm increases. However with more individuals the effect of the covariates decreases see figure \ref{fig:timedist_LR_cov_10ks_2e+05i_gpu4}.

\begin{figure}[h]
    \centering
    \includegraphics[width=9cm]{timedist_LR_cov_10ks_2000i_gpu4.png}
    \caption{LR distribution with 4 GPU 3 stream, 10k SNPs, 2k individuals}
...
\begin{figure}[h]
    ...
    \caption{LR distribution with 4 GPU 3 stream, 10k SNPs, 200k individuals}
...
gpu good, problem with scaling to 3-4 gpus
likely due to cpu cannot give enough calculations to them
moving cpu things to gpu would help
however cpu to gpu might not do much for 1-2 gpu since the bottlneck is the gpu side depending on data size

however cpu to gpu improvment good for that too since less cpu work cold mean pure cpu threads
if there is cpu power left could use separate cpu worker threads to fill the rest
cpu version is good to have too

solution:more stuff to gpu, the SVD and the recoding
cpu threads possible

another potential idea:
split out the snp reading to separate threads. add another queue.
hard to say if it would actually give speed, might not be worth it
it is likely be hidden by gpu calculations, especially if more work done on gpu

general gpu improvement would not give much since mostly cublas
needs to be revisited after more things move to gpu

for more speed->clusters

%TODO look back att efficent cuda part
not much to say, most things don't apply that much because mostly cublas
good to know for future stuff

also need better validation, permutation tests or bootstrap
divergence problems, nothing now but could be with recoding on GPU or bootstrap
idea for solution using pointer architmic

%cublas not that much harder than regular blas.
...
more statistics for non binary
if binary possibly take inspiration from g-g? lots of methods for filtering there
...
use clusters and move more to gpu next step
also need permutation tests or bootstrap, or other validation method
...
%TODO either update list of abbreviations or remove
...
\begin{tabular}{| l | c c c | c | c c c |}
  ...
  \hline  
\end{tabular}
\caption{The four cases of allele frequencies. Case 1 upper left, case 2 upper right, case 3 lower left, case 4 lower right}
