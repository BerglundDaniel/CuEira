...
\\
Other basic information such as the information about individuals, environment factors and SNPs are stored in simple classes where each individual, environmental factor or SNP is a single object. \emph{Enums} are used to store some of them, enums are a data type previously explained in section \ref{basic_program_term}.\\
\\
...
200k individuals extrapolate for GEISA. It was cut after about a hour and 1040 SNPs calculated. With shown in figure \ref{fig:geisa_vs_cueira_nocluster_10ksnps}, without shown in figure \ref{fig:geisa_vs_cueira_nocluster_10ksnps_exclude}

\begin{figure}[h]
    \centering
    \includegraphics[width=12cm]{geisa_vs_cueira_nocluster_10ksnps.png}
    \caption{Speedup GEISA vs CuEira. GEIRA 200 000 individuals extrapolated. 10 000 SNPs.}
    \label{fig:geisa_vs_cueira_nocluster_10ksnps}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=12cm]{geisa_vs_cueira_nocluster_10ksnps_exclude.png}
    \caption{Speedup GEISA vs CuEira. Without 200 000 individuals data. 10 000 SNPs.}
    \label{fig:geisa_vs_cueira_nocluster_10ksnps_exclude}
\end{figure}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=12cm]{geisa_vs_cueira_cluster_10ks.png}
    \caption{Speedup GEISA vs CuEira. 10 000 SNPs.}
    \label{fig:geisa_vs_cueira_cluster_10ks}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=12cm]{geisa_vs_cueira_cluster_10ki.png}
    \caption{Speedup GEISA vs CuEira. 10 000 Individuals.}
    \label{fig:geisa_vs_cueira_cluster_10ki}
\end{figure}
...
For case 4 it is clear that Allele1 should be the risk by looking at an extreme case. The case is shown in table \ref{table:risk_allele_extreme_case}. Almost everyone in the case group has Allele1 while no one in the control group does. The MAF based definition and JERIA/GEISA sets Allele2 as the risk for this case while increase in frequency sets Allele1.
...
\begin{tabular}{| l | c c |}
  ...
  \hline  
\end{tabular}
\caption{Example frequencies of case 4 in table \ref{table:risk_allele_cases}}
\label{table:risk_allele_extreme_case}
\end{table}
