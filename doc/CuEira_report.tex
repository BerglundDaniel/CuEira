...
\\
To determine if the genetic risk is present for each individual one of the alleles needs to be chosen as the \emph{risk allele}. GEIRA and JEIRA determines the risk allele by comparing the MAFs of case and control\cite{geisa, uvehag_master_thesis}. If MAF of cases is greater or equal to the MAF of controls the minor allele is used as the risk otherwise the other allele\cite{geisa, uvehag_master_thesis}. In this thesis the risk allele will be defined as the allele that has higher frequency in cases than controls. It is similar to the definition using MAF in most cases. Appendix \ref{risk_allele_diff} has more detailed comparisons.\\
\\
...
In general retrieving data from memory is relatively slow compared to how fast the processor works\cite{introduction_hpc_hager,drepper2007cpumemory}. %TODO something about the  maginitude
...
%TODO facade interface, more pattern stuff? adapter, wrapper
...
%TODO builder pattern, adapter pattern
...
%TODO write about LR matrix inverse
The inverse of the information matrix, or Jacobian, on line 10 is generally not possible with normal matrix inverse because it is not defined for a general matrix. However the \emph{pseudo-inverse} is.  
...
The descriptions and structures here apply to both JEIRA and GEISA since their underlying structure is the same, however the focus is on GEISA. They are written in Java and uses Javas built-in functions for concurrency. Java was chosen because it is %TODO \cite{uvehag_master_thesis}
...
There are a few unit tests for some of the basic storage classes and the binary file reader.\\

\section{CuEira}
\label{cueira}
This section will explain the structure of CuEira and how it works starting with a general overview. Later the parts are explained and finally how the parts are combined to form the program.\\
\\
...
Factories are used were needed to help satisfy the dependency inversion principle.

Namespaces are used in CuEira to 

Comments are used very sparsely for several reasons. The names and could so be clear and readable so that they should not be needed and it is also easy to forget to update them.
%TODO add comment stuff to clean code?
...
\begin{description}
  ...
  \item[Boost] \hfill \\
  Boost is a collections of C++ libraries. For CuEira it is used for file handling, string operations and parsing the command line options.
  \item[CUDA] \hfill \\
...
\\
By changing an option in the main CMake file the program can be compiled with either single and double precision. 

%TODO more preproccessor variable stuff, PRECISION, DEBUG, CPU

More details on how to compile and use CuEira can be found in appendix \ref{compile_cueira}.
...
\\
Other basic information such as the information about individuals, environment factors and SNPs are stored in simple classes where each individual, environmental factor or SNP is a single object. \emph{Enums} are used to store some of them. An enum is a data type consisting of a set of named values. This is useful when the variable has a distinct set of values that makes more sense as text rather than something else. For instance in CuEira the individuals gender is stored as enum with the two values MALE and FEMALE. The named enum values are commonly write in only capital letters. The gender could be stored an int, however MALE is easier to read than an int value and it also avoids potential errors with incorrect values. An enum can only have one of it's possible values while an int is not restricted in the same way.\\
\\
The SNPs are stored in the class DataQueue. DataQueue provides the function \textbf{next()} that fetches a SNP from the queue and returns it. It has a lock so it can be shared among all worker threads.\\
\\
The EnvironmentFactorHandler is responsible for keeping track of the EnvironmentFactors and it's corresponding data. When constructed it updates the EnvironmentFactors variable type. This is done because it can sometimes be needed to know if a variable is binary or not. It also provides functions to access the EnvironmentFactors and the corresponding vectors.\\
\\
There is three data container classes, SNPVector, EnvironmentVector and InteractionVector. They all have the responsibility to keep track of the recoded data and to perform the recoding when their \textbf{recode()} function is used. The \textbf{recode()} function takes an Recode enum which tells it which type of recoding it shoudl perform. The SNPVector also holds the original SNP data. EnvironmentVector doesn't need to since EnvironmentFactorHandler has that responsibility and InteractionVector does not have any kind of original data.\\
\\
The DataHandler is responsible for keeping track of the current combination of data containers being used and to iterate to the next one when its \textbf{next()} function is called. When the \textbf{next()} function is used a couple of things happen. If all environment factors have been calculated together with the current SNP it deletes the SNPs information, asks the queue for a new one and reads its data and switches to the first environment factor. Otherwise it moves on to the next environment factor. It then updates the InteractionVector and the Statistics classes to create the contingency table and the allele frequencies. After that the next set of variables is ready to be used.

\subsection{File I/O}
%TODO coviariates outside too
CuEira reads five files that are in four different formats. The reading of all files except one is handled by the facade pattern class %TODO pattern name?
DataFilesReader. DataFilesReader and how it delegates is shown in figure\\% \ref{fig:data_files_reader}.\\ TODO
\\
Three of the files are binary Plink files, of which one is binary. The fam file has the information about the individuals, the bim file has the names of the SNPs and their alleles. The genetic information for each individual is in the binary bed file. More details on Plink files can be found in appendix \ref{plink_file}.\\
\\
...
\\
The BedReader class is outside DataFilesReader because it needs some of the information that is inside the other Plink files. It is also a different because it is the only reader that is continuously used during the whole execution of the program.\\
\\
There is only one writer, the ResultWriter, and as its name suggests its responsibility is to write the results. The results are written immediately and then discarded to save memory. There is an lock on the writing so several threads can share it. The results will therefore be written in the order they are completed. 

%TODO picture of the whole readers thing

\subsection{Initialisation of the variables, Recoding and Statistic Model}
The initialisation of the SNPVector and EnvironmentVector is done by calling the recoding with ALL\_RISK from the Recode enum. The \textbf{recode()} function calls different functions based on the enum. If it is ALL\_RISK it calls a function that copies the data from the vector holding the original data to vector that holds the data that will be used for the model. InteractionVector does recoding and initialisation the same way because in both cases the interaction data is the multiplication of the elements of the SNPVector and EnvironmentVector. InteractionVector recode is shown in algorithm \ref{alg:inter_recode}. SNPVector recode is shown in algorithm \ref{alg:snp_recode} and EnvironmentVector recode is shown in algorithm \ref{alg:env_recode}.\\
\\
For the additive statistical model all the elements in SNPVector and EnvironmentVector needs to be set to 0 when the corresponding element in InteractionVector is 1. Both have an function named \textbf{applyStatisticModel()} that does it. It takes the InteractionVector as an parameter and the algorithm is the same for both SNPVector and EnvironmentVector. The algorithm is shown in algorithm \ref{alg:app_stat_model}.\\
\\
...
%The statistics classes are responsible for calculating based on the results and to calculate the distribution of the groups, for instance the contingency table. 

\subsection{Wrappers}
CUDA and BLAS functions and interfaces are in C style. To make it easier to use and fit better with the rest of the programs they were put in wrappers. These wrappers are in C++ style and do the actual function calls to CUDA and BLAS. Since CUDA and BLAS contains a lot of various functions they were added as needed. There are several reasons to use wrappers. One is that some of the functions does not use exceptions but returns an error code instead. This forces the program to check these error codes. This is hidden by using a wrapper that can throw the correct exception based on the error code. Exceptions are also better for performance because it removes the need for if statements checking the returned error codes. The compiler can optimize better with checks for exceptions(e.g. try catch statements) because it can assume that the occurrence of exceptions is low. However usually checks for exceptions are not needed because they commonly signify a critical error.\\ %TODO cite exception performance
\\
...
CUBLAS is wrapped together with the kernels made for CuEira in KernelWrapper in a similar way to how MKL is wrapped in MKLWrapper. Other CUBLAS utility and CUDA functions are wrapped in CudaAdapter.\\
\\
The CUDA streams are managed by a Stream class and its factory. When the StreamFactory creates a new Stream object it creates a new CUDA stream, a new CUBLAS handle and associates the CUBLAS handle with the new stream. The Stream class has functions to retrieve them and if the Stream object is destroyed it destroys both the CUDA stream and the CUBLAS handle. Each Stream object is associated with an Device object. The Device class represents an device(i.e. GPU) and since a thread can only issue commands to one device at a time the Device class has functions to set it as the active one and check if itself is the device active or not. The outcomes of the individuals does not change so it can be shared between all streams on the device and the Device class is responsible for the vector with the outcomes on the device.\\
\\
...
\subsection{Model}
%TODO write model stuff

%TODO possibly move, write something about that it uses builder pattern as talked about in clean code stuff
...
A group of base classes

%TODO description list thing
...
ModelInformation. It has the ContingencyTable and AlleleStatistics

The basic ModelHandler has next to switch DataHandler to the next combination and to updated it's own pointers to the new data.

The next section will explain how the base classes here were extended to form the classes for the LR.

\subsubsection{Logistic Regression}
The LR model is implemented using the Model base classes talked about in the previous section. There is a CPU version and a GPU version using CUDA. Some classes are shared between them because of common elements. The structure is shown in figure \ref{fig:lr_inheritance}.
...
To make testing easier the LR algorithm is split into several protected functions that are then used in the \textbf{calculate()} function. For the CudaLogisticRegression some of these parts are done on the host. It is the parts that perform line 10 and 11 in algorithm \ref{alg:lr}. All the other parts of the LR algorithm are done by using CUBLAS or the kernels explained in section \ref{kernelwrapper}. The calculations for line 10 and 11 are in the LogisticRegression class so the code is shared between CpuLogisticRegression and CudaLogisticRegression. It is done on the host instead of the device because two reasons.\\
\\
The size of the vectors and matrices used there is small, their size is 4 + the number of covariates. The second reasons is that the inverse of the information matrix(variable \textbf{J} in the algorithm) is done by singular value decomposition(SVD). The reason for using SVD instead of normal matrix inverse was previously explained in section %\ref{}. %TODO
However CUBLAS does not provide an SVD function. There are other libraries that do, CULA Tools\cite{culatools} and MAGMA\cite{magma_2010}, however neither could be used.\\
\\
...
\subsection{Combining the Parts and Threading}
3 threads per GPU
...
n persons
%p*n*e storage of env
%p*n*c storage of cov
...
%TODO thread stuff, not needed mostly because so few things are shared. Most of them doesn't have any kind of state or anything that can be affected so different threads can't affect each other.
...
%performance measures, 1,2,3,4 gpus
%vary streams per gpu 1-6, on several gpus and on ones
...
%TODO actually case 4 probably not possible, point x=0 is at 0? x=1 is <0 so no point >0 after x>0
...
Each line after header is a SNP environment factor combination.
No specific order
Column Y contains an number of the SNP, sort based on it and gets same order as bim file

Multiplicative model is recoded

RiskAllele is according to first set, no recode

Contingency table stuff is recoded

\chapter{Bugs and Issues}

\section{Risk Allele}
\label{risk_allele_diff}
\label{geira_jeira_risk_allele}
%TODO fix risk allele stuff. comparasions
...
\data{caseMaxn and controlMaxn is the frequency for the cases and controls.\\
caseMax and controlMax is the name of the allele that has the highest frequency in cases and controls.}

\BlankLine \BlankLine

\If{$caseMaxn > controlMaxn$ \KwSty{and} $caseMax = controlMax$}{
  $riskAllele \longleftarrow caseMax$\;
}\ElseIf{$caseMaxn \le controlMaxn$ \KwSty{and} $caseMin = controlMin$}{
  $riskAllele \longleftarrow caseMin$\;
}\ElseIf{$caseMaxn > controlMaxn$ \KwSty{and} $caseMax \neq controlMax$}{
  $riskAllele \longleftarrow caseMax$\;
}\ElseIf{$caseMaxn \le controlMaxn$ \KwSty{and} $caseMin \neq controlMin$}{
  $riskAllele \longleftarrow caseMin$\;
}

\caption{GEIRA pseudo code to determine the risk allele}
\algorithmslist{GEIRA pseudo code to determine the risk allele}
...
\algorithmslist{JEIRA and GEISA pseudo code to determine the risk allele}
...
%TODO JEIRA risk allele
%TODO JEIRA wrong columns
%TODO badly documented output
%TODO plink is a mess
...
%TODO tests in separate execution file
